...
//备份发卡表
public class ReceptionVehicleInfoBak  extends  BeanModel{

	String orderID_s, vehicleID_s, vehicleCPG_s, model_s, color_s, interPhoneID_s, fieldID_s;
	Date createDate_t;
	...
	public String toString() {
		return "ReceptionVehicleInfo [orderID_s=" + orderID_s
				...
				+ vehicleCPG_s + ", model_s=" + model_s + ", color_s="
	 ...
	 * @since  CodingExample　Ver(编码范例查看) 1.1
	 */
	public void copyVI(ReceptionVehicleInfo receptionVehicleInfo){
		orderID_s = receptionVehicleInfo.getOrderID_s();
		vehicleCPG_s = receptionVehicleInfo.getVehicleCPG_s();
		model_s = receptionVehicleInfo.getModel_s();
		color_s = receptionVehicleInfo.getColor_s();
		weight_s = receptionVehicleInfo.getWeight_s();	
		maxWeight_s = receptionVehicleInfo.getMaxWeight_s();
		fuelDemand_s = receptionVehicleInfo.getFuelDemand_s();
		fuelConsumption_s = receptionVehicleInfo.getFuelConsumption_s();
		category_i = receptionVehicleInfo.getCategory_i();
		licensPlate_s = receptionVehicleInfo.getLicensPlate_s();
		brandType_s = receptionVehicleInfo.getBrandType_s();
		maxSpeed_s = receptionVehicleInfo.getMaxSpeed_s();
		vin_s = receptionVehicleInfo.getVin_s();		 
		this.createDate_t = DateUtil.getCurrentDate();
		this.createUser_s = receptionVehicleInfo.getCreateUser_s();
		
		vehicleCPG_s = receptionVehicleInfo.getVehicleCPG_s();
		interPhoneID_s = receptionVehicleInfo.getInterPhoneID_s();
		fieldID_s = receptionVehicleInfo.getFieldID_s();
		 
		 
 		resaveds1_s = receptionVehicleInfo.getResaveds1_s();
 		resaveds2_s = receptionVehicleInfo.getResaveds2_s();
 		resaveds3_s = receptionVehicleInfo.getResaveds3_s();
 		resaveds4_s = receptionVehicleInfo.getResaveds4_s();
 		resaveds5_s = receptionVehicleInfo.getResaveds5_s();
 		roidIds_s = receptionVehicleInfo.getRoidIds_s(); //道路ID集合 已"vv"作为分隔符   存储订单每辆车道路权限  2014-06-11
 		vehicleID_s = receptionVehicleInfo.getVehicleID_s();
		 	
		   

		  axleNumber_i = receptionVehicleInfo.getAxleNumber_i();
		
		
		  roadNames_s = receptionVehicleInfo.getRoadNames_s();//逻辑字段，用于存放roadID查寻出来的roadName
