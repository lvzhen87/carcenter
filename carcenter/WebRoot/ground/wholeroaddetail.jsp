   ...
   <div class="content-box"><!-- Start Content Box -->
		...
		<div class="content-box-header" style="width: 1040px;">
			<ul class="content-box-tabs" >
				...
				<li><a href="./wholeroadAction"  class="tab-content default-tab" >包场试验道路</a></li>
			...
			<div class="clear"></div>
		<form action="./wholeroadAction" id="wholeroadform" name="wholeroadform" method="post"></form>
	...
	<table class="dbgrid">
		<thead>
			<tr bgcolor="#E8F0FF">
			   ...
			   <th width="10%">是否退订</th>
			   <th width="6%">
			   		<a title="添加" href="./addWholeRoadAction"  >
           				<img src="images/icons/add.png" style="cursor:hand;"  alt="添加" />
        			</a> 
			   		<a title="刷新" href="./wholeroadAction" >
	       ...
	       <c:forEach items="${wholeroadlist}" var="wholeroadlist" varStatus="wholeroadlistStatus">
			...
			<tr style="${evenOddStyle}">
			  <td>${statusIndex+1 }</td>
			   ...
			   <td>
			   		<a title="退订" onclick="conf('${wholeroadlist.orderWholeRoadID_s}')"    >
	...
	<script type="text/javascript">
	function addItem(t){
		var winPar=window.showModalDialog("./addWholeRoadAction?orderWholeRoadID_s="+t, "", "dialogWidth=980px;dialogHeight=600px;toolbar=no;menubar=no;location=no;status=no;scroll=no;");
	 	//alert(winPar);
		if(winPar=="refresh"){
			//document.getElementById("wholeroadform").submit();
		}
	}
	function conf(s){
		var winPar=window.showModalDialog("./wholeRulesAction?orderWholeRoadID_s="+s, "", "dialogWidth=980px;dialogHeight=600px;toolbar=no;menubar=no;location=no;status=no;scroll=no;");
		if(winPar=="refresh"){
			document.getElementById("wholeroadform").submit();
		}
