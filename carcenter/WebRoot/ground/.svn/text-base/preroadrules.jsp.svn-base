 ...
 <div id="man_zone" style="height: 520px; width: 1080px;">
	<center>
  	<table width="60%">
		...
		<c:forEach items="${rulelist }" var="rlist">
			<tr>
				<td>* 超过${rlist.greaterHourNumber_i }小时，不满${rlist.lessHourNumber_i }小时，违约扣除比例是${rlist.breakPromiseDeduction_i }</td>
			</tr>
		</c:forEach>
  		</c:otherwise>
  		</c:choose>
  	...
  	<object  classid="clsid:0b6ed426-9e67-4cf3-99da-8a346a98e5c6" codebase="Setup1.msi"
  		...
  		<input type="hidden" id="orderPrepaymentRoadID_s" name="orderPrepaymentRoadID_s" value="${orderPrepaymentRoadID_s}"/>
  	...
  	<script type="text/javascript">
  		...
  		$.messager.confirm('确定', str, function(r){
  			...
  			$.post(
  				...
  				{
  					card:$("#card").val(),
  					orderPrepaymentRoadID_s:$("#orderPrepaymentRoadID_s").val(),
