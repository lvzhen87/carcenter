  ...
  <body style="padding: 0px;margin: 0px; width: 99%">
   <div class="content-box"><!-- Start Content Box -->
		...
		<div class="content-box-header" style="width: 1040px;">
			...
			<form action="./searchInfoDetailAction?new Date().getTime()" id="infoform" name="infoform" method="post"></form>
			<ul class="content-box-tabs" >
				<li><a href="./searchInfoDetailAction?new Date().getTime()" class="tab-content default-tab">试验基础信息</a></li> <!-- href must be unique and match the id of target div -->
		...
		<c:when test="${empty infolist}">
	       ...
	       <c:forEach items="${infolist}" var="infolist" varStatus="infolistStatus">
			...
			<tr style="${evenOddStyle}">
			   ...
			   <td>${infolist.startDate_s }~${infolist.endDate_s } </td>
			   <td>
					<a title="修改" onclick="modify('${infolist.customerID_s}')" >
