...
<style type="text/css">
 ...
 <div id="man_zone" >
	<form action="./saveWholeRoadAction" id="roadform" name="roadform" method="post" >
	 <table id="orderroadinfo" width="98%" >
        <tr >
        	...
        	<td width="30%" style="vertical-align: middle;">
        		...
        		<a href="./wholeroadAction" class="easyui-linkbutton" >
					返回
				</a>
        	</td>
        </tr>
...
<c:choose>
	<c:when test="${empty orderWholeRoadTimeSquan }">
	<p style="color:red;">请点击【查询时间段信息】，先查询具体时间可订阅信息，然后预定.</p>
		...
		<tr>
			<c:forEach items="${orderWholeRoadTimeSquan}" var="item" varStatus="status" begin="${8*num}" end="${8*num+7 }" >
...
function delChooseHour(box){
	...
	function submitRoadform0(){
		...
		*/
		$("#roadform").attr("action","./orderRoadWholeInfoAction");
		$("#roadform").submit();
	}
