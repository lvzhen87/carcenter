...
<script type="text/javascript">
		...
		function submitAddVehicleFrom(){
			...
			var invalidates="";
			//	Yepeng Add
			//var invalidate = "";$("#roadType").combobox('getValues')
			...
			for(i=0;i<facenum;i++){
				...
				alert($("#invalidate"+i).combobox('getValues'));
				if(!temp){
			...
			//alert(i);
					...
					alert(invalidates)
					//	Yepeng Add
					//invalidate += $("#invalidate").val() + ",";
					//alert("invalidate:" + invalidate);
			...
			$.post(
	  				...
	  				{	
	  					...
	  					if(num > 0){
		  					...
		  					var text5=" 失效时间：<input id='invalidate"+(num+i)+"' name='invalidate"+(num+i)+"' value='"+$("#invalidate").datebox("getValue")+"'  type='text' class='easyui-datebox' data-options='formatter:myformatter,parser:myparser' style='width:120px'/><font color='red'>*</font>";
		  					var text12=" <a title='删除' onclick='deletevehicle('"+(num+i)+"')' ><img src='images/icons/cross_circle.png' style='cursor:hand;'  alt='删除' /></a> ";
		  					var text9="	<br/>";
		  					var text11="</div>";
		  					//value='"+$("#invalidate").datebox("getValue")+"'
		  					...
		  					var targetObj = $("#div1").append(text10,text1,text2,text6,text3,text4,text5,text12,text11,text9);
