...
<form id="searchOrderForm" name="searchOrderForm" action="./searchOrderCostAction" method="post">
			<div style="margin: 0px 0px 0px 5px">
				...
				状态： <select name="states" class="easyui-combobox">
	...
	<thead>
		...
		<c:choose>
			<c:when test="${empty ordersList}">
				<tr><td colspan="8">没有找到相关记录.</td></tr>
			</c:when><c:otherwise>
		<!-- 遍历mapSess中存入的相应数据 -->
		<tbody>
        ...
        <tr style="${evenOddStyle}">
		   ...
		   <td><c:choose>
		   			...
		   			<c:when test="${orderslist.status_i==2}">
				   		预订已确认
		   			</c:when>
		   			<c:when test="${orderslist.status_i==3}">
				   		预订取消
		   			</c:when>
		   			<c:when test="${orderslist.status_i==5}">
				   		已进场
		   			</c:when>
		   			<c:when test="${orderslist.status_i==6}">
				   		试验进行中
		   			</c:when>
		   			<c:when test="${orderslist.status_i==7}">
				   		试验结束
		   			</c:when>
		   			<c:when test="${orderslist.status_i==8}">
				   		订单已结算
		   			</c:when>
		   			<c:otherwise>
		   ...
		   <td >
				${orderslist.employee.customerUserName_s}
           </td>
           <td>
           	<a href="./costDetailAction?orderID=${orderslist.orderID_s}" title="费用明细" >
           		<img alt="费用明细" src="images/money_dollar.png" style="cursor: hand;" >
           	</a>
           </td> 
		</tr>
       	</c:forEach>
    	<tr>
    		...
    		<td colspan="4" style="text-align: right;">
    			<div style="display: inline;width: 400px;float: right;padding-right: 5px;">
					<a href="./searchOrderCostAction?currentPage=0&states=${states}" >首页</a>&nbsp;
					<a href="./searchOrderCostAction?currentPage=${currentPage-1<=0?0:currentPage-1}&states=${states}">前一页</a>&nbsp;
					<a href="./searchOrderCostAction?currentPage=${currentPage+1>=maxPage?totalPage:currentPage+1}&states=${states}">后一页</a>&nbsp;
					<a href="./searchOrderCostAction?currentPage=${maxPage-1}&states=${states}">末页</a>&nbsp;&nbsp;
				</div>
    		</td>
    	</tr>
	</tbody>
