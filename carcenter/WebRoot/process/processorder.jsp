 ...
 <body style="padding: 0px;margin: 0px; width: 99%">
   ...
   <!-- 条件查询div -->
	<div style="height: 40px;padding: 5px;margin: 0px 5px 0px 10px">
		<!-- 根据条件，查询道闸信息 -->
		<form id="searchStationForm" name="searchStationForm" action="./searchprocessorderAction?time=new Date().getTime()" method="post">
			<div>
				...
				CPG牌照号：<input type="text" id="resaveds2" name="resaveds2" value="${resaveds2 }" />
				...
				订单状态： <select name="states" id="states" class="easyui-combobox">
								...
								<option value="1"  >订单未确认</option>
								<option value="2"  >订单已确认</option>
							 
								<option value="5"  >已进场</option>
								<option value="6"  selected="selected">试验进行中</option>
								<option value="7"  >试验结束</option>
								<option value="8"  >订单已结算</option>
						</select>
				<!--  
	...
	<table  title="状态信息" id="manageordertab"  border="0" width="98%" style="margin: 0px 0px 0px 10px">
		...
		<c:forEach items="${OrderList}" var="orderlist" varStatus="orderlistStatus">
			...
			<tr style="${evenOddStyle}">
				...
				<td>
					<a id="passimg" class="passcheck" title="出入道路详情" href="./searchprocessAction?orderID_s=${orderlist.orderID_s}" >
            		<img src="images/icons/icon_info.png" style="cursor:hand;" />
           			 </a>
           			 <a id="passimg" class="passcheck" title="进出查询" href="./searchstatisticAction?orderID_s=${orderlist.orderID_s}" >
            		<img src="images/money_dollar.png" style="cursor:hand;" />
           			 </a>
				</td>
			</tr>
		</c:forEach>
				...
				<tr>
					...
					<td width="50%" style="text-align: right;">
						<div style="display: inline;width: 300px;float: right;padding-right: 5px;">
							<a href="./searchprocessorderAction?currentPage=0&orderID_s=${orderID_s}&orderName_s=${orderName_s}&customerName=${customerName}&startDate=${startDate}&endDate=${endDate}&states=${states}" target="manFrame" >首页</a>&nbsp;
							<a href="./searchprocessorderAction?currentPage=${currentPage-1<=0?0:currentPage-1}&orderID_s=${orderID_s}&orderName_s=${orderName_s}&customerName=${customerName}&startDate=${startDate}&endDate=${endDate}&states=${states}">前一页</a>&nbsp;
							<a href="./searchprocessorderAction?currentPage=${currentPage+1>=maxPage?totalPage-1:currentPage+1}&orderID_s=${orderID_s}&orderName_s=${orderName_s}&customerName=${customerName}&startDate=${startDate}&endDate=${endDate}&states=${states}">后一页</a>&nbsp;
							<a href="./searchprocessorderAction?currentPage=${maxPage-1}&orderID_s=${orderID_s}&orderName_s=${orderName_s}&customerName=${customerName}&startDate=${startDate}&endDate=${endDate}&states=${states}">末页</a>&nbsp;&nbsp;
