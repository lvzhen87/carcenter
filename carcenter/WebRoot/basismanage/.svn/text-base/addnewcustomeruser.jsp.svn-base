 ...
 <div id="man_zone" style="height: 520px;">
  	...
  	<table width="90%">
  		<tr>
  			...
  			<td width="30%"> 
  				 <select id="customer" class="easyui-combobox">
	  					<c:forEach items="${customerListForUser}" var="cuslist">
	  						<option value="${cuslist.customerID_s }" <c:if test="${cuslist.customerID_s == vcustomeruser.customerid_s}">  selected="selected" </c:if> >${cuslist.customerName_s}</option>
	  					</c:forEach>
  					</select>
  			</td>
  		...
  		<tr>
  			...
  			<td > <input type="text" id="identityCard_s" name="identityCard" value="${vcustomeruser.identitycard_s}" onblur="Checkfun.isNull(this,'身份证号',20)"/> &nbsp;&nbsp;<font color="red">*</font></td>
  		</tr>
  		<tr>
  	...
  	<script type="text/javascript">
  		$("#addUserBt").click(function (){
  			...
  			$("#usertype").val($("#userType_i").combobox('getValue'));
  			//校验略
