...
public class OrderRoadWholeAction extends CapgActionSupport {
	...
	public String execute(){
		...
		try {
			...
			mapSess.put("orderWholeRoadTimeSquan", preRoadDataInfo);
			//产出当前道路的 最大容量值
			...
			e1.printStackTrace();
			setErrmsg("查询包场时间段数据，操作错误!【"+e1.getMessage()+"】");
			return ERROR;
		}
		return SUCCESS;
	}
	...
	public String saveWholeRoad(){
		try {
			List<OrderWholeRoad>roadsList =new ArrayList<OrderWholeRoad>();
			//根据时间段，用v成分成多个对象
			String ts[]=timeQuantum.split("v");
			for (int i = 0; i < ts.length; i++) {
				OrderWholeRoad owr=new OrderWholeRoad();
			...
			IorderWholeRoadService owSer=new OrderWholeRoadServiceImpl();
			owSer.OperationOrderWholeRoad(roadsList, 1);
		} catch (Exception e) {
			e.printStackTrace();
